...
const TouristsSpots = () => {
    ...
    // console.log(tourism[0].avgCost)
    // const num = parseInt(tourism[0].avgCost)
    // console.log(num)
    ...
    const handleFilter = filtertxt =>{
       console.log(filtertxt)
     let sortedCard = [...filter]
     if(filtertxt === 'avgCost'){
        sortedCard.sort((a,b)=> b.num - a.num)
     }
     setfilter(sortedCard);
    ...
    return (
        <div >
                ...
                <ul className="p-2 shadow menu text-center dropdown-content z-[1] bg-base-100 rounded-box w-52">
                        <li onClick={()=> handleFilter("avgCost")} >  <a>avgCost</a></li>
            ...
            <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 space-y-8">
                {
                    tourism.map(tour=>  <div key={tour._id} className="card card-compact w-96 bg-base-100 p-2 shadow-xl">
